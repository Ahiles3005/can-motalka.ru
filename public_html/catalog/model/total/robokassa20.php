...
class ModelTotalRobokassa20 extends Model {
	...
	{		
		...
		)  
		{
			...
			$total_data[] = array( 
        		...
        		'value'      => $cost,
				'sort_order' => $this->config->get('robokassa20_sort_order')
			);
	...
	{
		$out_summ = 0;
		$url = "https://auth.robokassa.ru/Merchant/WebService/Service.asmx/GetRates?MerchantLogin=".
		...
		if( $page )
		{
			...
			if( isset($simpleXml->Groups->Group) )
			{
				for($i=0; $i<count($simpleXml->Groups->Group); $i++ )
				{
					for($e=0; $e<count($simpleXml->Groups->Group[$i]->Items->Currency); $e++ )
					{
						if( $simpleXml->Groups->Group[$i]->Items->Currency[$e]['Label'] == $currency )
						{
							$out_summ = $simpleXml->Groups->Group[$i]->Items->Currency[$e]->Rate['IncSum'][0];
						}
					}
				}
			}
		...
		return (float)$out_summ;
	}	
	
	function getRatesHash() 
	{
		$HASH = array();
		$url = "https://auth.robokassa.ru/Merchant/WebService/Service.asmx/GetRates?MerchantLogin=demo&IncCurrLabel=&OutSum=1000&Language=ru";
