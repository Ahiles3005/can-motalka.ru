...
function overlay_block(selector) {
    ...
    if (obj.length > 0) {
        ...
        jQuery("#" + selector + " .simplecheckout_overlay")
            .css({
                'background' : 'url('+simple_path+'catalog/view/image/load1.gif) no-repeat center center',
                'opacity' : 0.4,
                'position': 'absolute',
                'width': blockWidth,
                'height': blockHeight,
...
function overlay_button() {
    ...
    if (jQuery('.s-wait').length == 0) {
        jQuery('#simplecheckout_button_confirm').after('<span class="s-wait">&nbsp;<img src="'+simple_path+'catalog/view/theme/default/image/load2.gif" alt="" /></span>');        
    }
}

function overlay_button_remove() {
    jQuery('#simplecheckout_button_confirm').attr('disabled', false);
    jQuery('.s-wait').remove();
}
...
function scroll_to_error() {
    if (!simple_steps) {
        ...
        if (jQuery('#simplecheckout_customer .simplecheckout-error-text:visible').length > 0 || jQuery('#simplecheckout-customer-fields .simplecheckout-error-text').length > 0 || jQuery('.simplecheckout-warning-block:visible').length > 0) {
            ...
            jQuery('html, body').animate({ scrollTop: offset - 70 }, 'slow');
        }
