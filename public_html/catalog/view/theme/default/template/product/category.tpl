	  ...
	  <div class="cart-box">
        <div class="cart-button" style="display:flex;flex-direction:row;align-items:flex-start;flex-wrap:wrap">
		  <input type="button" value="<?php echo $button_cart; ?>" onclick="addToCart('<?php echo $product['product_id']; ?>');" class="btns c-button" style="flex-basis:35%" />
			  <?php if ($product['upc'] && $config_product_button_ali) { ?><a href="<?php echo $product['upc']; ?>" class="btns prod-cart ali" target="_blank" rel="noopener noreferrer" style="display: block;width: auto;height:35px;line-height:35px;padding: 0 10px!important;">Заказать с AliExpress</a><?php } ?>
