...
class ControllerAccountOrder extends Controller {
	...
	public function info() { 
		...
		if ($order_info) {
            ...
            // attribute
            $attributes_t = $this->model_catalog_product->getProductAttributes($product['product_id']);
				...
				$options = $this->model_account_order->getOrderOptions($this->request->get['order_id'], $product['order_product_id']);
         		...
         		foreach ($options as $option) {
                    ...
                    if (!empty($category_name['price_sign'])) {
                        $np = $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value'])."/".html_entity_decode($category_name['price_sign']);
                    } else {
                        $np = $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']);
                    }
                    // price sign         		
