...
function Wg($Mci=false){global$Mu,$Mk,$Mzi;if($Mzi!="cache"&&$Mu&&lc('Mnt')&&!lc('fm')&&!strpos($Mk,"itemap")){global$Mvq;$Mwb=preg_match("/google|yandex|bing|hthou/i",$Mvq);if(!$Mwb&&!$Mzi){$Mzi="cache";$Mbg='';if(!empty($_SERVER["REMOTE_ADDR"]))$Mbg=$_SERVER["REMOTE_ADDR"];Wjn($Mvq,array("cache","",$Mbg,"Это бот, который не относится к важным поисковикам",1));Wjm();}
if($Mwb&&!$Mzi){$Mzi="allow";$Mbg='';if(!empty($_SERVER["REMOTE_ADDR"]))$Mbg=$_SERVER["REMOTE_ADDR"];Wjn($Mvq,array("unblock","",$Mbg,"Это бот относится к важным поисковикам",1));Wjm();}
if($Mwb&&!empty($_SERVER["REMOTE_ADDR"])&&!is_file(DIR_CACHE."lightning/bots/".$_SERVER["REMOTE_ADDR"])){$Mpd=gethostbyaddr($_SERVER["REMOTE_ADDR"]);$Mwb=preg_match("/google|yandex|msn|hthou/i",$Mpd);if(!$Mwb){Wjn($_SERVER["REMOTE_ADDR"],array("unblock","",$Mvq,"Этот IP представляется поисковым ботом, но ведет на "."<a target='_blank' href='http://$Mpd'>$Mpd</a>",1));Wjm();}else{Whe(DIR_CACHE."lightning/bots");file_put_contents(DIR_CACHE."lightning/bots/".$_SERVER["REMOTE_ADDR"],"$Mpd\n$Mvq");}}}
if($Mzi=="cache"){header("X-Lightning: non-cached pages access deny");header("Retry-After: 86400");global$Mj;$Mj="Нет в кеше, проверьте позже";http_response_code(503);Wjo();exit;}
...
if(Whj())Whk($Mg,$Mwf);global$Mvq;if($Mg&&!headers_sent()&&(empty($_COOKIE["res_pushed"])and!preg_match("/^((?!chrome|android).)*safari/i",$Mvq))){$Mvh=$Mg;Wu($Mvh);if(lc('eg')=="optimal"&&!lc('ge')){$Maf=strpos($Mvh,"</style><link id='li_df'");if($Maf){header("Last-Modified:");$Mvh=str_replace("font-face","fontface",substr($Mvh,0,$Maf)).substr($Mvh,$Maf);$Mg=$Mvh;}}
...
if(strpos($Mvm,"/")===false)return$Mvm;$Mcj="";if(preg_match("/[\"',;\s]/",$Mvm[0])){$Mcj=$Mvm[0];$Mvm=substr($Mvm,1);}
if(substr($Mvm,0,4)=="url("){$Mcj.="url(";$Mvm=substr($Mvm,4);}
