<?php if(!empty($Mz)){if($Mz=="n"){require_once"tau.php";if(!empty($_GET["op"])){$Me=$_GET["op"];header("Access-Control-Allow-Origin: *");if($Me=="hide")Wa($_GET["id"]);elseif($Me=="show")Wa($_GET["id"],true);elseif($Me=="delete")Wb($_GET["id"],false);elseif($Me=="test"){$Mlm=Wc($_GET["id"]);if($Mlm===2)echo"UPDATE";elseif($Mlm)echo"OK";}
}else Wd();exit;}
if($Mz=="io"){require"optima.php";OptimizeTest();}
if($Mu&&$Mz!="access"){exit;}
...
if($M_o){$Mir->query("DELETE FROM ".DB_PREFIX."lightning_mail WHERE mail_id=".$M_o["mail_id"]);header("X-OpenCart-Lightning-Gen: Sending mail #".$M_o["mail_id"]);ob_start();register_shutdown_function('Wax');$Mh_=Wj("to\";s:", '";',$M_o["mail"]);$Mh_=Wj(':"','',$Mh_);file_put_contents(DIR_LOGS."lightning_mail.log",date("Y-m-d G:i:s")."  Sending mail #".$M_o["mail_id"]." to $Mh_\n",FILE_APPEND);require DIR_SYSTEM."library/mail.php";if(file_exists(DIR_SYSTEM."library/mail/mail.php"))require DIR_SYSTEM."library/mail/mail.php";if(file_exists(DIR_SYSTEM."library/mail/smtp.php"))require DIR_SYSTEM."library/mail/smtp.php";$M_p=unserialize($M_o["mail"]);$M_p->send();file_put_contents(DIR_LOGS."lightning_mail.log",date("Y-m-d G:i:s")."  Mail #".$M_o["mail_id"]." to $Mh_ sent successfully\n",FILE_APPEND);header("X-OpenCart-Lightning: Sent ok");exit;}
...
function Wq($Mbo=false){global$Mf,$Mz,$Maq;header("HTTP/1.1 200 OK",true,200);header("Status: 200");if(function_exists("header_remove")){header_remove("Content-Encoding");header_remove("Set-Cookie");header_remove("Location");}else{header("Content-Encoding: none");header("Set-Cookie:");}
...
require_once"beta.php";if(!$Mbr)$Mbr=$Mk;$Mk=false;if($Maq['Mbz'])foreach($Maq['Mbz']as$Mbs=>$Mbe){$Mbe=str_replace("&amp;","&",$Mbe);if($Mte and!$Maq['id']and!$Mtf){$Mbv=round(($Maq['Mha']-count($Maq['Mbz']))/$Maq['Mha']*100);file_put_contents(Wa.'ar',$Mbv,LOCK_EX);}
...
function Wem($Mcy){if(strpos($Mcy,'?'))$Mcy=Wak('','?',$Mcy).'?'.preg_replace('/[abcdef\d]/i','',Wak('?','',$Mcy));return$Mcy;}
...
function Was(&$Mg){if(!lc('ef')and!lc('eg'))return;global$Mam,$Mv;if($Mam or$Mv)return;Wu($Mg);if(!strpos($Mg,"<body"))return;$Mg=Wau("<link rel=\"preload\"*>",'',$Mg);$Mg=Wau("<link href=\"catalog/view/javascript/bootstrap/css/bootstrap.min.css\" rel=\"preload\" as=\"style\" />",'',$Mg);global$Mh,$Md;$Mmi=$Mh;if($Md){$Mmi=str_replace("http://","https://",$Mmi);}
$Mdo=lc('dh');$Mqp="";$Mdp="";$Mdq="";global$Mxf;$Mxf=array();if(lc('ef')and!Wey('dw')){$Mgw=lc('ef');$Mdq.="<script>window.performance.mark('Start JS Execution');</script>";$Mdr=Wcw("<script","</script>",$Mg);foreach($Mdr as$Mcz){$Mcz=substr($Mcz,strpos($Mcz,'>')+1);if(substr(trim($Mcz),0,4)=="<!--"){$Mne=substr(trim($Mcz),4);if(substr($Mne,-5)=="//-->")$Mne=substr($Mne,0,-5);elseif(substr($Mne,-3)=="-->")$Mne=substr($Mne,0,-3);$Mne=Wip("<!--*-->",'',$Mne);$Mg=str_replace($Mcz,$Mne,$Mg);}}
...
if(Wav('gf',$Mcy)||($Mgw!="aggressive"&&strpos($Mcy,"//")!==false&&!strpos($Mcy,$Mst))){$Mdq.=Wfs($Mds);$Mds=array();$Mdq.=$Mcz;continue;}
$Mds[]=$Mcy;}
...
if(lc('eg')){$M_s=array();$M_t="";$Mpj=lc('eg')!="safe";$Mdx=Wat("<link",">",Wip("<!--*-->",'',Wip("<noscript*</noscript>",'',$Mg)));$Mds=array();foreach($Mdx as$Mbc){$Mrx=strtolower($Mbc);if(!strpos($Mrx,"stylesheet")){continue;}
...
$Md_=str_replace("@font-face{","\n@font-face{font-display:swap;",$Md_);$Md_=str_replace("font-display:swap;font-family:'icomoon","font-family:'icomoon",$Md_);$Md_=str_replace("font-display:swap;font-family:'FontAwesome","font-family:'FontAwesome",$Md_);file_put_contents($Mdy,$Md_,LOCK_EX);}
...
if($Mpj and file_exists($Mdz)){$Mdp="<link id='li_df' rel='preload' as='style' href='".$Mmi."image/cache/lightning/".$Mda.".css'/>".$Mdp;if(true){$Mdp="<style>".file_get_contents($Mdz)."</style>".$Mdp;}else{$Mdp="<link rel='stylesheet' href='".$Mmi.str_replace(DIR_IMAGE,"image/",$Mdz)."'/>".$Mdp;}
...
if($M_t)$Mdp="\n<style>\n".$M_t."\n</style>\n".$Mdp;}
$Mcj="";foreach($Mxf as$Mcy)$Mcj.="<link rel='preload' as='script' href='".$Mcy."'/>";$Mdp=$Mcj.$Mdp;$Maf=strpos($Mg,"</head>");$Mek=strpos($Mg,"<style");if($Mek&&$Mek<$Maf){$Maf=$Mek;$Mel=strpos($Mg,"<head");$Mek=strpos($Mg,"<!-",$Mel);if($Mek&&$Mek<$Maf)$Maf=$Mek;}
$Mqs=strpos($Mg,"<base");if($Mqs>$Maf)$Maf=strpos($Mg,">",$Mqs)+1;while(ctype_space($Mg[$Maf-1]))$Maf--;if(substr($Mg,$Maf-10,10)=="<noscript>")$Maf-=10;$Mg=substr($Mg,0,$Maf).$Mdp.substr($Mg,$Maf);$Maf=strrpos($Mg,"</body>");if(!$Maf)$Maf=strlen($Mg);$Mg=substr($Mg,0,$Maf).$Mdq.substr($Mg,$Maf);}
function Wh_($Mfd){global$Mq,$Mn,$Mw,$My,$Mxb;if(!empty($Mq["compare"])and$Mn['ap']){if(!We($Mw))return false;Ww($Mn['ap'],"0",$Mfd);}
...
function Wax(){global$Mz,$M_o,$Mcq,$Mq;if($M_o){@ob_end_clean();if(Wdr)echo"OX";else echo"OK";return;}
$Mdt=http_response_code();if($Mz=="gen"||$Mz=="gens")if($Mdt>200){if(function_exists("header_remove"))header_remove("Location");else header("Location:");http_response_code(200);Wa_("");global$Maq;if($Maq){Waz();global$Mtf;Wq(!empty($Maq['Mbz'])or$Mtf);}
return;}
...
global$light_ob,$Mk,$Mam;if($Mam)exit;if(!$light_ob)return;$Mem='';Wab(true);while($Maf=ob_get_level()){$Mem=ob_get_contents().$Mem;@ob_end_clean();if(ob_get_level()==$Maf)break;}
Wbg();if(!trim($Mem)){global$Maq;if($Maq){Wa_(false);Wq(!empty($Maq['Mbz']));}
echo$Mem;return;}
Wu($Mem);if(!rand(0,100))Wgc($Mem);global$Mxb;if($Mxb&&$Mdt)if($Mdt!=404)$Mxb=false;if(!$Mxb)if(strpos($Mem,"//cdn.ampproject.org/v0.js")||$Mdt==404){Wz($Mem);return;}
...
global$Mw,$Mn;$Mec=!lc('Mnt');if(isset($_GET["db"]))$Mec=true;if($Mo_=error_get_last())if($Mo_["type"]==1||$Mo_["type"]==4)$Mec=true;if(!$Mec){if($light_ob==1)$Mec=true;elseif(!empty($Mq['_'])&&!lc('e'))$Mec=true;elseif(!empty($Mq["customer_id"])&&!lc('ab'))$Mec=true;elseif(!empty($Mq["wishlist"])&&!lc('ac'))$Mec=true;elseif(!empty($Mq['_'])||!empty($Mq["customer_id"])||!empty($Mq["wishlist"])){if(We($Mw)){if(!empty($Mq['_'])and(!$Mn['ah']||$Mq['_']===true))$Mec=true;elseif(!empty($Mq["customer_id"])&&!$Mn['z'])$Mec=true;}else$Mec=true;}}
...
$Mgc=unserialize($Mgb);@file_put_contents(Wa,stripslashes($Mgc['lc']),LOCK_EX);$Mgd=unserialize(stripslashes($Mgc['Mgd']));foreach($Mgd as&$Mge)$Mge=str_replace("xy".'Mf_'."xy","SELECT",$Mge);@file_put_contents(Wa.'Mgd',serialize($Mgd),LOCK_EX);}else{Wbi("Ошибка подключения к серверу Lightning");return false;}}
...
function Wbo($Mht,&$Mem,&$Mbq=false){if($Mht=="common/cart"||strpos($Mht,"odule/cart"))Wbr($Mem);global$Mh;if(!$Mh)$Mh=W_("config")->get("config_url");$Mce=$Mht;if(strpos($Mce,".tpl"))$Mce=Wak("template/",".tpl",$Mce);$Mce=str_replace("_mobile","",$Mce);$Mce=str_replace("_tablet","",$Mce);if($Maf=strpos($Mce,"emplate/"))$Mce=substr($Mce,$Maf+8);global$Mrj;if(!$Mrj)$action[0]=$Mce;else$action=end($Mrj);if(!empty($_GET["li_module"])){$Mrk=$action[0];$Mrk.=Wez($action[1]);$Mem="<span style=' background: yellow; color: black; padding: 3px; margin: 3px'>Start: ".$Mrk."</span>".$Mem."<span style=' background: #CCC; color: black; padding: 3px; margin: 3px'>End: ".$Mce."</span>";}
global$Mz,$Mab,$Ma,$light_ob,$Mvb;if(!$Ma)return false;if(!($Mz=="gens"and$Mab)&&($Mvb or Wav('b_',$action[0])or We_('b_',$action[1]))&&!Wai()&&$light_ob>1&&!strpos($Mem,"</body>")){$Mvb=false;$Mrk=$action[0];try{if($action[1])$Mrk.='='.serialize($action[1]);}catch(Exception$Mey){}
global$Meo;$Meo[]=$Mrk;$Mem="<div id='liaj".count($Meo)."'></div>";array_pop($Mrj);return false;}
array_pop($Mrj);if(empty($_COOKIE['az']))return false;if($Mce=="common/cart"||$Mce=="module/cart"){$Mem='<Wca>'.$Mem.'<Wcb>';if($_COOKIE['az']==1){$Mhu[]=W_("language")->get("decimal_point");$Mhu[]=W_("language")->get("thousand_point");$Mem.='<Wcc>'.base64_encode(serialize($Mhu)).'<Wcd>';}
...
function Wbr($Mem=false){if(!lc('Mnt')or lc('gg'))return;$Mij=W_("cart")->countProducts();global$Mq;if(!$Mij)unset($Mq['_']);elseif(lc('e')){if(function_exists("iconv"))$Mq['_']=iconv("UTF-8","UTF-8//IGNORE",$Mem);else$Mq['_']=$Mem;}
else$Mq['_']="cart";}
...
function Wfc($Mgj){static$Msb;if($Msb||$Mgj!="action_event")return;$Msb=true;if(lc('fd'))return;global$registry;if(empty($registry))return;$Msc=new Light_Event($registry);$registry->set("event",$Msc);}
...
public function register($Msj,$Mtc,$Msi=0){if(defined("VERSION")&&VERSION==="3.0.2.0"&&$Msj==="controller/*/after"&&lc('gh')){$Mtc="event/language/after";}
...
return true;}
function Wfb(&$Mbq){global$Mz,$Mab;if($Mz and!lc('t')and empty($Mab)){unset($Mbq["user_id"]);}
if(!empty($_COOKIE['az'])or Wdr)W_("config")->set("config_maintenance",0);if(!empty($_COOKIE['az'])&&!defined("Wfd"))define("Wfd",1);global$Mo;if($Mo)$Mbq=$Mo;$GLOBALS['Mq']=&$Mbq;}
function Wgt(){if(lc('t'))return false;global$Mr;if($Mr and!lc('u'))return false;return lc('Mnt');}
function Waq($Mdg){if(!empty($GLOBALS['Ma']['er']))return;static$Mbt;if($Mbt>10)return;$Mdl=microtime(true);$Mtt=Wgk();if($Mtt=='ex')return;static$Mue;if($Mdg===$Mue)return;$Mue=$Mdg;if(substr($Mdg,0,6)=="cache/")$Mdg=DIR_IMAGE.$Mdg;if(substr($Mdg,0,strlen(DIR_SYSTEM)-7)!=substr(DIR_SYSTEM,0,-7))$Mdg=substr(DIR_SYSTEM,0,-7).$Mdg;if(!file_exists($Mdg)){$Mdg=str_replace("%20",' ',$Mdg);if(!file_exists($Mdg))return;}
if(lc('hb')&&!lc('hc')&&!strpos($Mdg,"/cache/"))return;clearstatcache(true,$Mdg);$Msq=filemtime($Mdg)% 76543;if($Msq==1)return;if($Mtt=='es'&&!$Msq){if(filemtime($Mdg)>time()-3600||file_exists(DIR_IMAGE."cache/lightning/optimize.active"))return;}
Wab();Wih($Mdg,floor((filemtime($Mdg)+76542)/76543)*76543);Wbg();clearstatcache(true,$Mdg);if($Mtt=='ew'){Wgf($Mdg);$Mbt+=microtime(true)-$Mdl;return;}
...
function Whz($Mxc){if(lc('fl')or file_exists(DIR_APPLICATION."model/extension/module/redirect_manager.php")or file_exists(DIR_APPLICATION."controller/common/redirect.php"))return;global$Mu,$My,$Mxb;if($Mu){header("X-Lightning: fast 404 answer for bot");http_response_code(404);exit;}
...
global$light_modify_cart;if(!$light_modify_cart){Wbr("cart");return;}
if(VERSION<"2.0")$Mxm=$this->getChild("module/cart");else$Mxm=W_("load")->controller("common/cart");Wbr($Mxm);}
function Wjk(){global$Mz;return$Mz;}
...
function Wkh($Mce){return(!empty($_SERVER["HTTP_X_REQUESTED_WITH"])&&strpos($_SERVER["REQUEST_URI"],"j3_ajax")&&!in_array($Mce,explode(' ',"journal3/product/extras journal3/product/second_image journal3/product/countdown journal3/layout journal3/filter common/column_left common/column_right journal3/settings")));}
function Wkf($Mxs){if(!lc('ha'))return false;global$Mir;$M_q=serialize($Mxs);$Mh_=Wj("to\";s:", '";',$M_q);$Mh_=Wj(':"','',$Mh_);$Mir->query("INSERT INTO ".DB_PREFIX."lightning_mail SET mail='".$Mir->escape($M_q)."'");file_put_contents(DIR_LOGS."lightning_mail.log",date("Y-m-d G:i:s")."  Mail to ".$Mh_." saved to Lightning queue\n",FILE_APPEND);return true;}
function Wkj($M_w,$Mop,$M_x){if(!lc('hb'))return false;global$Mcq;if($Mcq!="product/product"&&$Mcq!="journal3/product")return;$config=Wc_("config");static$M_y,$M_z;if(!$M_y){if(lc('gh')){$M_y=$config->get("theme_journal3_image_thumb_width");$M_z=$config->get("theme_journal3_image_thumb_height");}else{$M_y=$config->get("theme_".$config->get("config_theme")."_image_popup_width");$M_z=$config->get("theme_".$config->get("config_theme")."_image_popup_height");}
if(!$M_y){$M_y=$config->get("config_image_popup_width");$M_z=$config->get("config_image_popup_height");}
if(!$M_y){$M_y=400;$M_z=400;}}
if($Mop<$M_y||$M_x<$M_z)return;$M__=str_replace(' ',"%20",$M_w);if(Wc_("request")->server["HTTPS"]){return$config->get("config_ssl")."image/".$M__;}else{return$config->get("config_url")."image/".$M__;}}