...
class ControllerInformationInfo extends Controller {
	public function index() {  
		...
		if (isset($this->request->get['info_id'])) {
			$information_id = (int)$this->request->get['info_id'];
		} else {
			$information_id = 0;
		}
		
		$information_info = $this->model_catalog_info->getInformation($information_id);
		...
		if ($information_info) {
      		...
      		$this->data['breadcrumbs'][] = array(
				...
				'href'      => $this->url->link('information/info', 'info_id=' .  $information_id),      		
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/information/info.tpl')) {
				$this->template = $this->config->get('config_template') . '/template/information/info.tpl';
			} else {
				$this->template = 'default/template/information/info.tpl';
			}
