	...
	private function getFilterSql() {
		...
		foreach ($links as $link => $parameter) {
			...
			if (isset ($this->request->post[$link])) {
				...
				if ($link == 'store' || $link == 'related' || $link == 'download' || $link == 'filter') {
					$sql .= " AND p.product_id " . $not . " IN (SELECT product_id FROM " . DB_PREFIX . $parameter['table'] . " WHERE " . $link . "_id IN (" . implode (', ', $data) . "))";
				} else if ($link == 'layout') {
					$where = array ();
					...
					if ($where) {
						$sql .= " AND p.product_id " . $not . " IN (SELECT product_id FROM " . DB_PREFIX . $parameter['table'] . " WHERE " . implode (' OR ', $where) . ")";
					} else {
						if ($not) {
							$sql .= " AND p.product_id " . $not . " IN (SELECT product_id FROM " . DB_PREFIX . $parameter['table'] . ")";
						}
					}
				} else if ($link == 'reward') {
					$where = array ();
					...
					$sql .= " AND p.product_id " . $not . " IN (SELECT product_id FROM " . DB_PREFIX . $parameter['table'] . " WHERE image IN (" . implode (', ', $images) . ") AND sort_order IN (" . implode (', ', $sort_orders) . "))";
					...
					if ($sql_or) {
						$sql .= " AND p.product_id " . $not . " IN (SELECT product_id FROM " . DB_PREFIX . $parameter['table'] . " WHERE " . implode (' OR ', $sql_or) . ")";
					}
					...
					foreach ($data as $attribute) {
						...
						if (isset ($attribute['attribute_description'])) {
							foreach ($attribute['attribute_description'] as $language_id => $value) {
								...
								if ($value['text'] != '') {
									$sql_lang[] = "(language_id = " . (int) $language_id . " AND text = '" . $this->db->escape($value['text']) . "')";
								}
							}
						}
					...
					if ($sql_or) {
						...
						if ($count) {
							$sql .= " AND p.product_id IN (SELECT `product_id` FROM `" . DB_PREFIX . $parameter['table'] . "` GROUP BY `product_id` HAVING COUNT(*) = '" . ((isset ($languages))? ($count_or * count ($languages)) : $count_or) . "')";
						}
					}
				} else if ($link == 'option') {
					$sql_or = array ();
					...
					if ($sql_or) {
						$sql .= " AND p.product_id " . $not . " IN (SELECT `product_id` FROM `" . DB_PREFIX . "product_option` WHERE " . implode (' OR ', $sql_or) . ")";
					}
				}
			} else {
				if ($not) {
		...
		foreach ($links as $link => $parameter) {
			...
			if ($sql_or) {
				...
				if ($count) {
					$sql .= " AND p.product_id IN (SELECT `product_id` FROM `" . DB_PREFIX . $parameter['table'] . "` GROUP BY `product_id` HAVING COUNT(*) = '" . $count_or . "')";
				}
			} else {
				if ($not) {
					$sql .= " AND p.product_id " . $not . " IN (SELECT `product_id` FROM `" . DB_PREFIX . $parameter['table'] . "`)";
