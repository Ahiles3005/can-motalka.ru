...
<script type="text/javascript" src="view/javascript/jquery/superfish1.7.4/js/superfish.js"></script>
          ...
          <div class="tab-content">
	                    ...
	                    <td>
	                      ...
	                      <select name="search_engine_options[fields][<?php echo $field; ?>][search]" class="form-control">
	                        ...
	                        <option value="1"  <?php echo (isset($options['fields'][$field]['search']) && $options['fields'][$field]['search'] == '1') ? 'selected="selected"' : ""; ?>><?php echo $text_enabled; ?></option>
											...
											<td>
 	                      <input type="checkbox" name="search_engine_options[fields][<?php echo $field; ?>][use_morphology]" value="1" <?php echo isset($options['fields'][$field]['use_morphology']) && $options['fields'][$field]['use_morphology'] ? "checked=checked" : ""; ?> />
	                    </td>
											
											<td>
 	                      <input type="checkbox" name="search_engine_options[fields][<?php echo $field; ?>][contains]" value="1" <?php echo isset($options['fields'][$field]['contains']) && $options['fields'][$field]['contains'] ? "checked=checked" : ""; ?> />
	                    </td>
            ...
            <div id="tab-exclude-words" class="tab-pane">
              ...
              <ul id="exclude-words-languages" class="nav nav-tabs">
	                ...
	                <li><a href="#exclude-words-language<?php echo $language['language_id']; ?>" data-toggle="tab"><img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?></a></li>
								<?php } ?>
              </ul>
            ...
            <div id="tab-replace-words" class="tab-pane">
              ...
              <ul id="replace-words-languages" class="nav nav-tabs">
								<?php foreach ($languages as $language) { ?>
	                <li><a href="#replace-words-language<?php echo $language['language_id']; ?>" data-toggle="tab"><img src="view/image/flags/<?php echo $language['image']; ?>" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?></a></li>
								<?php } ?>
              </ul>
...
function stop() {
  ...
  stopped = 1;
	...
	$.get('index.php?route=module/search_engine/stop&token=<?php echo $token; ?>');	
	
	$.get('index.php?route=module/search_engine/getTotals&token=<?php echo $token; ?>', function( data ) {
...
function process() {
		...
		$.ajax({
			url: 'index.php?route=module/search_engine/add&token=<?php echo $token; ?>',
			type: 'post',
			dataType: 'json',
