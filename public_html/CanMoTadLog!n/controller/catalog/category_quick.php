...
class ControllerCatalogCategoryQuick extends Controller { 
	...
	protected function getForm() {
		...
		// Products title
		$this->data['text_title_products'] = $this->language->get('text_title_products');		
		...
		if (isset($this->request->get['category_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
      		...
      		// product title
      		$this->data['product_title'] = $this->model_catalog_category_quick->getCategoryProductTitle($this->request->get['category_id']);
      		$category_attrs = $this->model_catalog_category_quick->getCategoryAttribute($this->request->get['category_id']);
      		// product title
    	}

    	//product title - attribute
    	if(!empty($category_attrs)) {
			foreach ($category_attrs as $category_attrs_result) {
				$this->data['category_attrs'][] = array(
					'attr_id'	=> $category_attrs_result['id'],
					'name'        	=> str_replace(':', '', $category_attrs_result['name'])
				);
			}
		}
    	// product title - attribute
		...
		if (isset($this->request->post['price_sign'])) {
			$this->data['price_sign'] = $this->request->post['price_sign'];
		} elseif (!empty($category_info)) {
			$this->data['price_sign'] = $category_info['price_sign'];
		} else {
			$this->data['price_sign'] = '';
		}

		if (isset($this->request->post['catname'])) {
			$this->data['catname'] = $this->request->post['catname'];
		} elseif (!empty($category_info)) {
			$this->data['catname'] = $category_info['catname'];
		} else {
			$this->data['catname'] = '';
		}

		if (isset($this->request->post['brandlink'])) {
			$this->data['brandlink'] = $this->request->post['brandlink'];
		} elseif (!empty($category_info)) {
			$this->data['brandlink'] = $category_info['brandlink'];
		} else {
			$this->data['brandlink'] = '';
		}
