...
class ControllerCatalogProductQuick extends Controller {
	...
	protected function getListing($product_id) {
		...
		$this->data['product_image'] = $this->model_tool_image->resize($this->model_catalog_product_quick->getProductIMG($this->request->get['product_id']), 80, 80);
		$this->data['product_no_image'] = $this->model_tool_image->resize('no_image.jpg', 80, 80);
		...
		if (isset($this->request->post['ozon'])) {
      		$this->data['ozon'] = $this->request->post['ozon'];
    	} elseif (!empty($product_info)) {
