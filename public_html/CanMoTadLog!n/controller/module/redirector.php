...
class ControllerModuleRedirector extends Controller {
  ...
  public function index() {
    $data = $this->language->load($this->module); //1.5
    // $this->init_module();
    $on_page = 20;
    $page = 1;
    $data['a'] = $this->a;
    ...
    //1.5
    $data['ht'] = strip_tags($this->language->get('heading_title'));
    $this->template = $this->module . '.tpl';
  ...
  private function init_module() {
    $this->language->load($this->module); //1.5
    ...
    if($a_number && isset($response_type[0]) && $response_type[0] != 'Notice'){
      ...
      $this->editSettingValue('unixml', 'unixml_key', $a_number); //1.5
    }else{
  ...
  //1.5
	public function editSettingValue($code = '', $key = '', $value = '', $store_id = 0) {
		if (!is_array($value)) {
			$this->db->query("UPDATE " . DB_PREFIX . "setting SET `value` = '" . $this->db->escape($value) . "', serialized = '0'  WHERE `group` = '" . $this->db->escape($code) . "' AND `key` = '" . $this->db->escape($key) . "' AND store_id = '" . (int)$store_id . "'");
		} else {
			$this->db->query("UPDATE " . DB_PREFIX . "setting SET `value` = '" . $this->db->escape(json_encode($value)) . "', serialized = '1' WHERE `group` = '" . $this->db->escape($code) . "' AND `key` = '" . $this->db->escape($key) . "' AND store_id = '" . (int)$store_id . "'");
		}
	}
	//1.5
}
