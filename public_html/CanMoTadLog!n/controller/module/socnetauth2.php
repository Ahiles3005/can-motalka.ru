...
class ControllerModulesocnetauth2 extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) 
		{
			...
			{
				$this->model_module_socnetauth2->updateSetting('config', 
			...
			if( !empty($this->request->post['stay']) )
			{
				...
				$this->response->redirect( str_replace("&amp;", "&", $this->url->link('module/socnetauth2', 'token=' . $this->session->data['token'].'&tab='.$tab.'&tab2='.$tab2, 'SSL') ));
				/* end 1505 */
			}
			else
			{
				$this->redirect( str_replace("&amp;", "&", $this->url->link('extension/module', 'token=' . $this->session->data['token'], 'SSL') ));			
			}
		}
		...
		{
			$this->data['socnetauth2_widget_name'] = $this->custom_unserialize( 
		...
		else
		{
			...
			{
				$this->data['socnetauth2_widget_name'][$language['language_id']] = $this->config->get('socnetauth2_widget_name_default');
			}
		}
		...
		$count_icons = $this->model_module_socnetauth2->getCountEnabledSocnets();
		...
		for($i=1; $i<=9; $i++)
		{
			...
			{
				$html = "<table class='sna_bline_table'>\n<tr>"."\n";
				
				for($e = 1; $e<=$i; $e++)
				{
					$html .= "<td><a href='{link_".$e."}'><img src='{img_".$e."}' /></a></td>\n";
				}
			...
			{
				$html = "<table class='sna_lline_table'>\n<tr>"."\n";
		...
		elseif( $this->config->has('socnetauth2_odnoklassniki_status') )
		{ 
			$this->data['socnetauth2_odnoklassniki_status'] = $this->config->get('socnetauth2_odnoklassniki_status');
		}
		else
		{
			$this->data['socnetauth2_odnoklassniki_status'] = 0;
		}
		...
		if (isset($this->request->post['socnetauth2_label'])) {
			...
			$this->data['socnetauth2_label'] = array();
		}
		...
		$this->template = 'module/socnetauth2.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
