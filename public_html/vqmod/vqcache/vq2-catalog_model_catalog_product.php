...
class ModelCatalogProduct extends Model {
	...
	public function getFeaturedProducts($data = array()) {
		if ($this->customer->isLogged()) {
			$customer_group_id = $this->customer->getCustomerGroupId();
		} else {
			$customer_group_id = $this->config->get('config_customer_group_id');
	...
	public function getCategoryProductTitle($category_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_title WHERE category_id = '" . (int)$category_id . "'");

		//$query2 = "SELECT * FROM " . DB_PREFIX . "product_title WHERE category_id = '" . (int)$category_id . "'";

//$file = 'people.txt';
//file_put_contents($file, $query2 , FILE_APPEND | LOCK_EX);
 ...
 if (function_exists('Wdn') and ($r=Wdn(func_get_args()))>=0) return $r; // Lightning 
	...
	// DATE AVAILABLE FOR FILTER PRO
	public function getDateAvailable($id) {
		$product_data = array();

		$query = $this->db->query("SELECT date_available FROM " . DB_PREFIX . "product WHERE product_id=" . (int)$id);

		foreach ($query->rows as $result) {
			$product_data[$result['date_available']] = $this->getProduct($result['date_available']);
		}
